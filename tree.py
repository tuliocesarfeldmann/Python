...
class Tree:
    ...
    def inserir(self, v):
        ...
        if self.root == None:
            ...
            while True:
                anterior = atual
                if atual.item == novo.item:
                    print('Esse número já existe na arvore!')
                    break
    ...
    def gerarArvoreAleatoria(self, alt):
        altura = self.altura(self.root)
        while alt > altura:
            self.inserir(randint(0, 100))
            altura = self.altura(self.root)
    ...
    def imprimirGraficamente(self, arr):
        idx = 0
        folhas = 0
        arr.insert(idx, self.root.item)
        while folhas < self.folhas(self.root):
            if type(arr[idx]) == int:
                no = self.buscar(arr[idx])
                if no.esq != None:
                    arr.insert((2*idx) + 1, no.esq.item)
                else:
                    arr.insert((2*idx) + 1, '')
                if no.dir != None:
                    arr.insert((2*idx) + 2, no.dir.item)
                else: 
                    arr.insert((2*idx) + 2, '')
                idx+=1
