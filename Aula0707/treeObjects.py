...
class Tree:
    ...
    def inserir(self, v):
        ...
        if self.root == None:
            ...
            while True:
                anterior = atual
                if v.getNome() <= atual.item.getNome():  # ir para esquerda
                    atual = atual.esq
                    if atual == None:
                        anterior.esq = novo
                        return
    ...
    def buscar(self, chave):
        ...
        while atual.item.getNome() != chave:  # enquanto nao encontrou
            if chave < atual.item.getNome():
    ...
    def remover(self, v):
        ...
        filho_esq = True
        # ****** Buscando o valor **********
        while atual.item.getNome() != v:  # enquanto nao encontrou
            pai = atual
            if v < atual.item.getNome():  # caminha para esquerda
                atual = atual.esq
    ...
    def gerarArvoreAleatoria(self, alt, id):
        while self.altura(self.root) < alt:
            self.inserir(Pessoa(id, g.geraSobrenome(), g.geraNome(), g.geraIdade()))
            id+=1
    
    def arvoreParaLista(self, atual, arr):
        if atual != None:
            arr.append(atual.item)
            self.arvoreParaLista(atual.esq, arr)
            self.arvoreParaLista(atual.dir, arr)
        return arr
    ...
    def caminhar(self):
        ...
        if self.root != None:  # se arvore nao esta vazia
            print(" Valor minimo: %s" % (self.minn().item.getNome()))
            print(" Valor maximo: %s" % (self.maxx().item.getNome()))